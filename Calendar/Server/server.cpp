...
int main()
{
	...
	//start listening at the socket, accept a maximum queue of 5  waiting connections
	listen(socket_fd, 5);
	//while(1) keeps the server running forever
	while(1)
	{
		...
		//this is a request, so remove any expired events, as per the requirement
		remove_expired_events();
		//get the username from the message
		get_username_from_message(message, user_name);
		//get user from username
		cur_user = get_user_from_username(user_name);
		//if username not found and this is not a add request, send 'u' indicating user not found and abort
		...
		//handle according to cases
		//in every case, reply 'f' if operation failed and 's' if successfull
		switch(message[32])
		{
			case('a'):		//add event
			{
			...
			case('d'):			//get all events for the date
			{
